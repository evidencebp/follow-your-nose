...
checkstyle_jar = '~/Downloads/checkstyle-8.41.1-all.jar' # new version
config='~/src/follow-your-nose/code/config/follow_your_nose_checks.xml'

checkstyle_cmd_template = \
    'java -cp {jar} com.puppycrawl.tools.checkstyle.Main -c {config} {repos_path}{repo} > {output_path}{repo}.txt '

err_dirs = []
#repos = os.listdir(path)
repos = ['checkstyle' #, 'batfish'
 ]
for repo in repos:
    try:
